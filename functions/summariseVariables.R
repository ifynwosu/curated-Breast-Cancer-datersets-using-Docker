summariseVariables <- function(metadata) {
  ...
  if ((ncol(numMatrix) > 2)) {
    ...
    if (!(is.null(add_to_char))) {
      numMatrix <- numMatrix %>% anti_join(add_to_char)
    }
  } else (numMatrix = NULL)
  ...
  if ((ncol(charMatrix) > 2)) {
    ...
    pivot_longer(3:ncol(charMatrix),
                 ...
                 values_to = "Value") %>%
    group_by(experiment_ID, Variable) %>%
    summarize(Unique_summary = paste(unique(Value), collapse=", "),
    Num_unique = length(unique(Value)))
  ...
  if (!is.null(charMatrix)) {
    charMatrix <- (rbind(charMatrix, add_to_char)) %>% dplyr::select(experiment_ID, Variable, Unique_summary, Num_unique)
  }
