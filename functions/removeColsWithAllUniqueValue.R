removeColsWithAllUniqueValue <- function(metadata) {
  ...
  excludeColumn = c("experiment_ID", "sampleID", "sex", "node", "relation", "hormone receptor status", "population", "therapy")
  ...
  metadata <- metadata %>% dplyr::select(-all_of(cols_to_remove))
