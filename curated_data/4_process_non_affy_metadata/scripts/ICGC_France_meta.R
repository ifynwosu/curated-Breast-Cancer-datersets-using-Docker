...
combined_df <- list(donor_BRCA_FR, donor_exposure_BRCA_FR, donor_family_BRCA_FR, donor_therapy_BRCA_FR, specimen_BRCA_FR) %>%
  ...
  dplyr::select(-starts_with("relationship_")) %>%
  distinct() %>%
  dplyr::filter(icgc_donor_id %in% BRCA_FR) %>%
  dplyr::rename(Sample_ID = icgc_donor_id) %>%
  mutate(Dataset_ID = "BRCA_FR", .before = Sample_ID)

#summarise metadata variables
varSummary <- summariseVariables(combined_df)

if (nrow(varSummary$numSummary) >= 1) {
  write_tsv(varSummary$numSummary, file.path("/Data/metadata_summaries/ICGC_BRCA_FR_num.tsv"))
}

if (nrow(varSummary$charSummary) >= 1) {
  write_tsv(varSummary$charSummary, file.path("/Data/metadata_summaries/ICGC_BRCA_FR_char.tsv"))
}


print("Writing BRCA_FR to file!")
write_tsv(combined_df, paste0(data_dir, "ICGC_BRCA_FR.tsv"))
