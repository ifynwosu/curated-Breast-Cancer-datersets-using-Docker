...
combined_df <- donor_BRCA_KR %>%
  ...
  mutate(Dataset_ID = "BRCA_KR", .before = Sample_ID)

#summarise metadata variables
varSummary <- summariseVariables(combined_df)

if (nrow(varSummary$numSummary) >= 1) {
  write_tsv(varSummary$numSummary, file.path("/Data/metadata_summaries/ICGC_BRCA_KR_num.tsv"))
}

if (nrow(varSummary$charSummary) >= 1) {
  write_tsv(varSummary$charSummary, file.path("/Data/metadata_summaries/ICGC_BRCA_KR_char.tsv"))
}

print("Writing BRCA_KR to file!")
write_tsv(combined_df, paste0(data_dir, "ICGC_BRCA_KR.tsv"))
